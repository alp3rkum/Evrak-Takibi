...
{
    public partial class GelenFatura : Form
    {
        ...
        public GelenFatura()
        {
            ...
            //{
            //    MessageBox.Show("Satış Raporu yüklenmemiş.", "Satış Raporu Yüklenmemiş", MessageBoxButtons.OK, MessageBoxIcon.Information);
            //    //return;
            //}
            InitializeComponent();
        ...
        private void GelenFatura_Load(object sender, EventArgs e)
        {
            ...
            if (!(Form1.satisRaporu == null))
            {
                ...
                using (ExcelPackage package = new ExcelPackage(Form1.satisRaporu))
                {
                    ...
                    {
                        if (worksheet.Cells[row, 3].Value.ToString().Equals("A", StringComparison.OrdinalIgnoreCase))
                        {
            ...
            else
            {
                ...
                {
                    query = $"SELECT * FROM dbo.Fatura WHERE alis_satis = 0"; //alış faturalarını getir
                    connection.Open();
        ...
        private void button_getir_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    if (text_faturano.Text.Length > 0)
                    {
                        query = $"SELECT * FROM dbo.Fatura WHERE eid LIKE '%@eid%' AND alis_satis = 0";
                        connection.Open();
                        using (SqlCommand command = new SqlCommand(query, connection))
                        {
                    ...
                    {
                        query = $"SELECT * FROM dbo.Fatura WHERE musteri_ad LIKE '%@ad%' alis_satis = 0";
                        connection.Open();
                        using (SqlCommand command = new SqlCommand(query, connection))
                        {
                    ...
                    {
                        query = $"SELECT * FROM dbo.Fatura WHERE (eid LIKE '%@eid%' OR musteri_ad LIKE '%@ad%') AND alis_satis = 0";
                        connection.Open();
                        using (SqlCommand command = new SqlCommand(query, connection))
                        {
                            command.Parameters.AddWithValue("@eid", text_faturano.Text);
        ...
        private void button_delete_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                {
                    faturaSelected = false;
                    string query = "DELETE FROM dbo.Fatura WHERE alis_satis = 0";
                    string connectionString = "Data Source=(localdb)\\MSSQLLocalDB;Initial Catalog=evraktakibi;Integrated Security=True";
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();
        ...
        private void CreateFatura(DataGridViewRow row)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(insertQuery, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@alis_satis", 0);
